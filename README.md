...
список доступных команд находится в директории `sh/`. Читайте комментарии внутри файлов.

# Для фронтэнд разработчиков

На компьютер фронтэнд разработчика необходимо скопировать файл 
`docker-compose-frontend.yml` под именем `docker-compose.yml`. 
Назначить в нем директорию, где хранятся изображения и запустить командой:
